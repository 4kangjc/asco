...
#define ASCO_LOG_LEVEL(logger, level) \
    if (logger->getLevel() <= level) \
        asco::LogContextWrap(std::make_unique<asco::LogContext>(logger, level, \
        __FILE__, __func__, __LINE__, asco::GetThreadElapse(), asco::GetThreadId(), \
        asco::GetFiberId(), asco::GetTimeMs() / 1000, asco::GetThreadName())).getSS()

#define ASCO_LOG_TRACE(logger)   ASCO_LOG_LEVEL(logger, asco::LogLevel::TRACE)
#define ASCO_LOG_DEBUG(logger)   ASCO_LOG_LEVEL(logger, asco::LogLevel::DEBUG)
#define ASCO_LOG_INFO(logger)    ASCO_LOG_LEVEL(logger, asco::LogLevel::INFO )
#define ASCO_LOG_WARN(logger)    ASCO_LOG_LEVEL(logger, asco::LogLevel::WARN )
#define ASCO_LOG_ERROR(logger)   ASCO_LOG_LEVEL(logger, asco::LogLevel::ERROR)
#define ASCO_LOG_FATAL(logger)   ASCO_LOG_LEVEL(logger, asco::LogLevel::FATAL)

#define ASCO_LOG_FMT_LEVEL(logger, level, fmt, args...) \
    if (logger->getLevel() <= level) \
        asco::LogContextWrap(std::make_unique<asco::LogContext>(logger, level, \
        __FILE__, __func__, __LINE__, asco::GetThreadElapse(), asco::GetThreadId(), \
        asco::GetFiberId(), asco::GetTimeMs() / 1000, asco::GetThreadName())).getContex()->format(fmt, ##args)

#define ASCO_LOG_FMT_TRACE(logger, fmt, args...)   ASCO_LOG_FMT_LEVEL(logger, asco::LogLevel::TRACE, fmt, ##args)
#define ASCO_LOG_FMT_DEBUG(logger, fmt, args...)   ASCO_LOG_FMT_LEVEL(logger, asco::LogLevel::DEBUG, fmt, ##args)
#define ASCO_LOG_FMT_INFO(logger,  fmt, args...)   ASCO_LOG_FMT_LEVEL(logger, asco::LogLevel::INFO,  fmt, ##args)
#define ASCO_LOG_FMT_WARN(logger,  fmt, args...)   ASCO_LOG_FMT_LEVEL(logger, asco::LogLevel::WARN,  fmt, ##args)
#define ASCO_LOG_FMT_ERROR(logger, fmt, args...)   ASCO_LOG_FMT_LEVEL(logger, asco::LogLevel::ERROR, fmt, ##args)
#define ASCO_LOG_FMT_FATAL(logger, fmt, args...)   ASCO_LOG_FMT_LEVEL(logger, asco::LogLevel::FATAL, fmt, ##args)

#define ASCO_LOG_ROOT()        asco::LoggerMgr::GetInstance().getRoot()
#define ASCO_LOG_NAME(name)    asco::LoggerMgr::GetInstance().getLogger(name)
